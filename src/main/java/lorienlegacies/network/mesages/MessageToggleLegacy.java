...
public class MessageToggleLegacy extends MessageBase
{
	...
	public void OnMessage(final MessageBase packet, Supplier<NetworkEvent.Context> context)
	{
		...
		ctx.enqueueWork(() ->
		{
			...
			Integer legacyXP = WorldLegacySaveData.get(player.getServer()).GetPlayerData().get(player.getUniqueID()).legacies.get(((MessageToggleLegacy)packet).legacy);
			if (legacyXP != null && legacyXP != 0)
				WorldLegacySaveData.get(player.getServer()).GetPlayerData().get(player.getUniqueID()).ToggleLegacy(((MessageToggleLegacy)packet).legacy);
		});
